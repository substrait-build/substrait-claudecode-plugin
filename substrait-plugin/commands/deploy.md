...
- `--env <name>` deploys to that environment instead of production:
  `bash "${CLAUDE_PLUGIN_ROOT}/scripts/substrait-deploy.sh" --env staging --watch`
...
- **Promote** one environment's live build into another — the way staging reaches
  ...
  `bash "${CLAUDE_PLUGIN_ROOT}/scripts/substrait-deploy.sh" promote --to production --from staging --watch`
